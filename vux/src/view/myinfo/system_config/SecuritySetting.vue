      ...
      <div v-transfer-dom>
        <popup v-model="showEdit" height="100%">
          ...
          <div v-if="editModel.currentEdit === 'loginPassword'">
            <group>
              <x-input type="password"  title="旧密码" :min="6" :max="20" ref="input1" v-model="resetLoginPassword.oldPassword" placeholder="请填写旧密码"></x-input>
              <x-input type="password"  title="新密码" :min="6" :max="20" ref="input2" v-model="resetLoginPassword.newPassword" placeholder="请输入新的密码"></x-input>
              <x-input type="password"  title="确认密码" :min="6" :max="20" ref="input3" v-model="resetLoginPassword.repeatPassword"  placeholder="请再次输入新密码"></x-input>
            </group>
          ...
          <div v-if="editModel.currentEdit === 'payPassword'">
            <group v-if="$store.state.currentUser.passwordReset">
            ...
            <group v-else>
              <x-input type="tel" title="手机号码" is-type="china-mobile" required ref="setPayPasswordInput1" v-model="setPayPassword.phone"></x-input>
              <x-input title="验证码" required class="weui-vcode" v-model="setPayPassword.smsCode" type="number" :min="4" :max="4" :show-clear="false" ref="setPayPasswordInput2">
                <x-button slot="right" type="primary" mini @click.native="getSmsCode" :disabled="smsCode.waiting">获取验证码{{smsCode.reget}}</x-button>
              </x-input>
              <x-input :is-type="isNumber" type="password" :max="6" required title="交易密码"  ref="setPayPasswordInput3" v-model="setPayPassword.payPassword"  placeholder="请输入交易密码"></x-input>
            </group>
          </div>
        </popup>
      </div>
